...
(function() {
  ...
  $.awesomeCropper = function(inputAttachTo, options) {
    ...
    input = function(type) {
//      return $("<input id='crop_" + $(inputAttachTo).attr('id') + "' type = \"" + type + "\" />");
    ...
    $cropSandbox = $('<canvas style="width:200px"></canvas><div style="width:199px;text-align:center;color:green">Preview</div>');
	//alert( settings.width );
    $cropSandbox.attr({
      width: settings.width,
      height: settings.height
    });
   // $container.append('<h3 width="105" height="120">ImagePreview</h3><br/>');
    $container.append($cropSandbox);
    $fileSelect = input('file').attr('style','width: 200px;');
	...
	$cropmessage = a('Note: Please Click on image to upload').attr({
      'href':"javascript:void(0);",
      'style':"color:red; position: absolute; left: 10px; font-size:13px;",
    });
      
    $applyButton = a('Upload').addClass('btn yes btn-primary uploadbutton').attr({
      'href':"javascript:void(0);",
      'id':"uploadbutton",
	  'style':"display:none;",
    });
    ...
    setOriginalSize = function(img) {
      ...
      tempImage.onload = function() {
		 ...
		 //alert(tempImage.width+'Invalid image size !'+ tempImage.height);
		
		 if(  tempImage.height < settings.height || tempImage.width < settings.width  )
		{
			$('.btn-danger').click();
			alert('Invalid image size, Please upload image size '+settings.width+' X '+settings.height+'.');
			//location.reload();		
			return false;			
		}
    ...
    drawImage = function(img, x, y, width, height) {
      ...
      var context, destHeight, destWidth, destX, destY, oHeight;
	   var oWidth, r, sourceHeight, sourceWidth, sourceX, sourceY;
...
//      console.log('==>port->'+viewPort);
//      console.log('==>src->'+$sourceIm.height());
   ...
   fileAllowed = function(name) {
    ...
    saveCrop = function() {
      ...
      $resultIm.attr('class', 'result_img');
    ...
    $(".uploadbutton").attr("style","display:none");
