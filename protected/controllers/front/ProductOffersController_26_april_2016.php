...
class ProductOffersController extends Controller
{
	...
	public function actionInsertpaymentinfo(){
		...
		if(empty($_POST['product_offer_id'])){
			echo 'Product offer id not found.';exit;			
		}
		$product_offer_id=$_POST['product_offer_id'];
		...
		$amount=$product_offer_model->offer->amount;
			
		$productinfo='Payment of offer product "'.$product_offer_model->product->product.'", amount(inclusive of all taxes) to be paid is '.$amount.' for duration '.date('d-m-Y',strtotime($product_offer_model->start_date)).' to '.date('d-m-Y',strtotime($product_offer_model->end_date));
		...
		$_POST['udf1']=$product_offer_model->poid;#To store product_offer_id 
		$_POST['amount']=$amount;
